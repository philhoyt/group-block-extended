...
const PRESET_VALUES = RATIO_PRESETS.map( ( p ) => p.value ).filter(
	( v ) => v !== '' && v !== 'custom'
);

/**
 * Determine the SelectControl value from the stored groupAspectRatio string.
 * @param {string} groupAspectRatio
 */
function getSelectValue( groupAspectRatio ) {
	if ( ! groupAspectRatio ) {
		return '';
	}
	if ( PRESET_VALUES.includes( groupAspectRatio ) ) {
		return groupAspectRatio;
	}
	return 'custom';
}
...
export default function AspectRatioControl( {
	...
	const hasMinHeight = useSelect(
		( select ) => {
			...
			return !! (
				block?.attributes?.style?.dimensions?.minHeight ||
				block?.attributes?.minHeight
			);
		},
		[ clientId ]
	);
