...
export default function LinkedGroupControl( {
	...
	const {
		isInsideQueryLoop,
		hasLinkedGroupAncestor,
		hasLinkedGroupDescendant,
	} = useSelect(
		( select ) => {
			...
			for ( const parentId of parents ) {
				...
				if (
					...
					( parent.attributes?.groupLinkUrl ||
						parent.attributes?.groupLinkToPost )
				) {
					linkedAncestor = true;
				}
			}
			...
			for ( const id of descendantIds ) {
				...
				if (
					...
					( block.attributes?.groupLinkUrl ||
						block.attributes?.groupLinkToPost )
				) {
					...
					{ ! groupLinkToPost && (
						<div className="gbe-link-control-wrapper">
							<LinkControl
								...
								onRemove={
									hasStaticLink ? handleLinkRemove : undefined
								}
								settings={ [
									{
										id: 'opensInNewTab',
					...
					{ isLinked && (
						<>
							...
							<TextControl
								...
								onChange={ ( value ) =>
									setAttributes( { groupLinkTitle: value } )
								}
							/>
